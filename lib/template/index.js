...
const mpComponentsPlugin = require( './plugins/mp_component' )
...
module.exports = function ( { template, script } ) {
  ...
  if ( script && script.content ) {
    const scriptAst = babel.transform( script.content, {
      ...
      plugins: [ componentsPlugin,mpComponentsPlugin ]
    } )

    components = scriptAst.metadata.components || {}
    mpComponents = scriptAst.metadata.mpComponents || []
  }
  ...
  Promise.all(
    ...
    compilerOptions.mpComponents = mpComponents
    ...
    imports.split( '\n' ).forEach( v => {
      if ( slotCache.imports.indexOf( v ) === -1 ) {
    ...
    if ( !slotsHookAdded ) {
      this._compiler.plugin( 'emit', ( compilation, callback ) => {
        const slotsContent = `<import src="../wxparse/index" />\n` + slotCache.imports.join( '\n' ) + '\n' + slotCache.slots.join( '\n' ) +
          `\n<template name="defaultSlot"></template>`

        const bytes = Buffer.byteLength( slotsContent, 'utf8' )

        // index + parent + child
        // emitFile cannot emit slots.wxml when template in index changes
        // so donot use emitFile here
